...
function SortableItem({ q, reviewId, idx }: { q: StudyQuestion; reviewId: string; idx: number }) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <li ref={setNodeRef} style={style} {...attributes} {...listeners}>
      <Item q={q} reviewId={reviewId} idx={idx} />
    </li>
  );
}

export default function DNDContext({ questions: initialQuestions, reviewId }: DNDContextProps) {
        ...
        <ul>
          {questions.map((q, idx) => (
            <React.Fragment key={q.id}>
              <SortableItem q={q} reviewId={reviewId} idx={idx} />
              <AddQuestionButton
                uploadId={reviewId}
      ...
      <DragOverlay>
        {activeQuestion ? (
          <div style={{ cursor: 'grabbing' }}>
            <Item q={activeQuestion} reviewId={reviewId} idx={activeIdx} />
          </div>
        ) : null}
      </DragOverlay>
    </DndContext>
