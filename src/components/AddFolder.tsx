...
export default function AddFolder() {
	...
	return (
		<li
			className={`flex items-center gap-5 translate-y-2 ${addFolderState ? "justify-between" : "justify-center"}`}
		>
			<div
				className={`overflow-hidden transition-all duration-300 ${addFolderState ? "w-full opacity-100" : "w-0 opacity-0"}`}
			>
				<input
					ref={inputRef}
					type="text"
			...
			>
				<div className={`transition-transform duration-300 `}>
					{addFolderState ? <Save size={20} /> : <FolderPlus size={28} />}
				</div>
			</button>
		</li>
	);
